...
router.post("/", auth("user"), async (req, res) => {
  try {
    ...
    // Use the spot's stored coordinates
    ...
    const listing = await Listing.create({
      ...
      booked: false // FIXED — schema default does NOT go in document
    });
...
router.get("/", async (req, res) => {
  try {
    ...
    // 1. Fetch listings for specific spot
    if (spotId) {
    ...
    // 2. Geospatial search requires lat/lng
...
router.delete("/:id", auth("user"), async (req, res) => {
  try {
    const listing = await Listing.findOneAndDelete({
      _id: req.params.id,
      ownerId: req.user.id, // restrict to the owner's listings
    });
